...
def update_all_messages():
    ...
    msg += f"<b>CPU:</b> {psutil.cpu_percent()}% | <b>RAM:</b> {psutil.virtual_memory().percent}% | <b>DISK:</b> {psutil.disk_usage('/').percent}%"
...
def sendStatusMessage(msg, bot):
    ...
    progress += f"<b>CPU:</b> {psutil.cpu_percent()}% | <b>RAM:</b> {psutil.virtual_memory().percent}% | <b>DISK:</b> {psutil.disk_usage('/').percent}%"
    with download_dict_lock:
        dlspeed_bytes = 0
        uldl_bytes = 0
        for download in list(download_dict.values()):
